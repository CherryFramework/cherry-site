 ...
 * @package Cherry
 */
if ( ! class_exists( 'Cherry_About_Author_Widget' ) ) {

	class Cherry_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'cherry widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'cherry' );
			$this->widget_id          = 'cherry_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'cherry' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'cherry' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'cherry' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'cherry' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'cherry' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'cherry' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'cherry' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'cherry' ),
					'label' => esc_html__( 'Link label', 'cherry' ),
				),
			);
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'cherry-author-avatar' );
...
add_action( 'widgets_init', 'cherry_register_about_author_widgets' );
function cherry_register_about_author_widgets() {
	register_widget( 'Cherry_About_Author_Widget' );
}
