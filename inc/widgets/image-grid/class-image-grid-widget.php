...
if ( ! class_exists( 'Cherry_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class Cherry_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Image Grid', 'cherry' );
			$this->widget_description = esc_html__( 'This widget displays images from post.', 'cherry' );
			...
			$this->settings           = array(
				...
				'category_name' => array(
					...
					'label'            => esc_html__( 'Select categories to show', 'cherry' ),
				...
				'post_sort' => array(
					...
					'value'   => 'date',
					'options' => array(
						'date'          => esc_html__( 'Publish Date', 'cherry' ),
						'title'         => esc_html__( 'Post Title', 'cherry' ),
						'comment_count' => esc_html__( 'Comment Count', 'cherry' ),
					),
					'label' => esc_html__( 'Post sorted', 'cherry' ),
				),
				'post_number' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Posts number', 'cherry' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number', 'cherry' ),
				),
				'items_padding' => array(
					'type'       => 'stepper',
					'value'      => '5',
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'cherry_image_grid_widget_args', $args, $instance );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if ( $post_taxonomy ) {
					$post_args = array(
						...
						'order'			=> apply_filters( 'cherry_order_image_grid_widget', 'DESC', $instance ),//ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				echo apply_filters(
					'cherry_image_grid_widget_before',
				...
				echo apply_filters( 'cherry_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'cherry_register_image_grid_widget' );
	function cherry_register_image_grid_widget() {
		register_widget( 'Cherry_Image_Grid_Widget' );
	}
}
