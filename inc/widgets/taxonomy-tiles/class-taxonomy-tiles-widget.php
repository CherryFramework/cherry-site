...
if ( ! class_exists( 'Cherry_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Cherry_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Taxonomy Tiles', 'cherry' );
			$this->widget_description = esc_html__( 'This widget displays images from taxonomy.', 'cherry' );
			$this->widget_id          = 'cherry-widget-taxonomy-tiles';
			...
			$this->settings           = array(
				'title' => array(
					...
					'label' => esc_html__( 'Widget title', 'cherry' ),
				),
				'terms_type' => array(
					'type'    => 'radio',
					'value'   => 'category',
				...
				'category'=> array(
					...
					'label'  => esc_html__( 'Select category to show', 'cherry' ),
				...
				'post_tag' => array(
					...
					'label'  => esc_html__( 'Select tags to show', 'cherry' ),
				...
				'description_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Description words length ( Set 0 to hide description. )', 'cherry' ),
				),
				'show_post_count' => array(
					...
					'options' => array(
						'show_post_count_check' => esc_html__( 'Show post count', 'cherry' ),
					),
				),
				'layout_type' => array(
					'type'    => 'radio',
					...
					'options' => array(
						'grid' => array(
							'label' => esc_html__( 'Grid', 'cherry' ),
							'slave' => 'layout_type_grid',
						),
						'tiles' => array(
							'label' => esc_html__( 'Tiles', 'cherry' ),
						),
					),
					'label' => esc_html__( 'Choose Layout Type', 'cherry' ),
				),
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number ( layout type grid )', 'cherry' ),
				...
				'items_padding' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Items padding ( size in pixels )', 'cherry' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				echo apply_filters( 'cherry_taxonomy_tiles_widget_before', '<div class="row grid ' . $layout_type . '-columns columns-number-' . $columns_number . '">', $instance );

				cherry_theme()->dynamic_css->add_style(
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$count = $this->utility->meta_data->get_post_count_in_term( array(
							...
							'sufix'   => _n_noop( '%s post', '%s posts', 'cherry' ),
				...
				echo apply_filters( 'cherry_taxonomy_tiles_widget_after', '</div>', $instance );
			}
...
add_action( 'widgets_init', 'cherry_register_taxonomy_tiles_widget' );
function cherry_register_taxonomy_tiles_widget() {
	register_widget( 'Cherry_Taxonomy_Tiles_Widget' );
}
