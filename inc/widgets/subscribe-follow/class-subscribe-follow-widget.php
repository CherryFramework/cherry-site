...
class Cherry_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'cherry' );
		$this->widget_id          = 'cherry_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'cherry' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'cherry' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'cherry' ),
				'label' => esc_html__( 'Subscribe Title', 'cherry' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'cherry' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'cherry' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'cherry' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'cherry' ),
				'label' => esc_html__( 'Subscribe submit label', 'cherry' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'cherry' ),
				'label' => esc_html__( 'Subscribe success', 'cherry' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'cherry' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'cherry' ),
				'label' => esc_html__( 'Follow Title', 'cherry' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'cherry' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'cherry' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'cherry' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'cherry' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'cherry' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'cherry' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'cherry' ),
					'top-center'    => esc_html__( 'Top Center', 'cherry' ),
					'top-right'     => esc_html__( 'Top Right', 'cherry' ),
					'center-left'   => esc_html__( 'Middle Left', 'cherry' ),
					'center'        => esc_html__( 'Middle Center', 'cherry' ),
					'center-right'  => esc_html__( 'Middle Right', 'cherry' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'cherry' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'cherry' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'cherry' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'cherry' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'cherry' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'cherry' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'cherry' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'cherry' ),
					'contain' => esc_html__( 'Contain', 'cherry' ),
					'auto'    => esc_html__( 'Auto', 'cherry' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'cherry' ),
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_cherry_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_cherry_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'cherry' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'cherry' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'cherry' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'cherry' ),
		);

		$this->nonce = wp_create_nonce( 'cherry-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'cherry' );
		}
	...
	public function get_social_nav() {
		return cherry_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = cherry_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'cherry-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'cherry_register_subscribe_follow_widgets' );
function cherry_register_subscribe_follow_widgets() {
	register_widget( 'Cherry_Subscribe_Follow_Widget' );
}
