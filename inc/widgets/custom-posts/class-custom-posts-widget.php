...
if ( ! class_exists( 'Cherry_Custom_Posts_Widget' ) ) {

	class Cherry_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'cherry' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'cherry' );
			$this->widget_id          = apply_filters( 'cherry_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'cherry_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = cherry_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'cherry' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format' => array(
							'label' => esc_html__( 'Post Format', 'cherry' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'cherry' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'cherry' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'cherry' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'cherry' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'cherry' ),
				),
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'cherry' ),
				),
				'meta_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'          => esc_html__( 'Date', 'cherry' ),
						'author'        => esc_html__( 'Author', 'cherry' ),
						'comment_count' => esc_html__( 'Comment count', 'cherry' ),
						'category'      => esc_html__( 'Category', 'cherry' ),
						'post_tag'      => esc_html__( 'Tag', 'cherry' ),
						'more_button'   => esc_html__( 'More Button', 'cherry' ),
					),
					'label' > esc_html__( 'Display post meta data', 'cherry' ),
				),
				'button_text' => array(
					'type'  => 'text',
					'value' => 'Read More',
					'label' => esc_html__( 'Post read more button label', 'cherry' ),
				),
			);
				...
				echo '<div class="custom-posts__holder row" >';
					...
					foreach ( $posts as $post ) {
						...
						$count = $this->utility->meta_data->get_comment_count( array(
							...
							'sufix'   => _n_noop( '%s comment', '%s comments', 'cherry' ),
						) );
	...
	add_action( 'widgets_init', 'cherry_register_custom_posts_widget' );
	function cherry_register_custom_posts_widget() {
		register_widget( 'Cherry_Custom_Posts_Widget' );
	}
}
