 ...
 * Sidebar position
 */
add_filter( 'theme_mod_sidebar_position', 'cherry_set_post_meta_value' );

/**
 * Header container type
 */
add_filter( 'theme_mod_header_container_type', 'cherry_set_post_meta_value' );

/**
 * Content container type
 */
add_filter( 'theme_mod_content_container_type', 'cherry_set_post_meta_value' );

/**
 * Footer container type
 */
add_filter( 'theme_mod_footer_container_type', 'cherry_set_post_meta_value' );
 ...
 * @return boolean
 */
function cherry_maybe_need_rewrite_mod() {
 ...
 * @return string
 */
function cherry_render_macros( $string ) {

	$macros = apply_filters( 'cherry_data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function cherry_render_icons( $content ) {
	$icons     = cherry_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'cherry_render_icons_callback', $content );
}
 ...
 * @return string
 */
function cherry_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function cherry_get_render_icons_set() {
	return apply_filters( 'cherry_render_icons_set', array(
 ...
 * @return string
 */
function cherry_render_theme_url( $url ) {
 ...
 */
function cherry_get_image_id_by_url( $image_src ) {
	global $wpdb;
 ...
 */
function cherry_post_formats_gallery() {
	$size = cherry_post_thumbnail_size();
	...
	$images = cherry_theme()->get_core()->modules['cherry-post-formats-api']->get_gallery_images( false );
 ...
 * @return bool
 */
function cherry_is_meta_visible( $meta, $context = 'loop' ) {

	if ( ! $meta ) {
		return false;
	}

	$meta_enabled = get_theme_mod( $meta, cherry_theme()->customizer->get_default( $meta ) );
 ...
 * @return array
 */
function cherry_post_thumbnail_size( $args = array() ) {
	$args = wp_parse_args( $args, array(
		'small'        => 'post-thumbnail',
		'fullwidth'    => 'cherry-thumb-l',
		'class_prefix' => '',
	) );

	$layout      = get_theme_mod( 'blog_layout_type', cherry_theme()->customizer->get_default( 'blog_layout_type' ) );
	$format      = get_post_format();
	$size_option = get_theme_mod( 'blog_featured_image', cherry_theme()->customizer->get_default( 'blog_featured_image' ) );
