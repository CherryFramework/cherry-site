...
if ( ! class_exists( 'Cherry_About_Widget' ) ) {

	class Cherry_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'cherry' );
			$this->widget_id          = 'cherry_widget_about';
			$this->widget_name        = esc_html__( 'About Cherry', 'cherry' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_id' => array(
					...
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Upload', 'cherry' ),
					'value'              => '',
					'label'              => esc_html__( 'Logo:', 'cherry' ),
				),
				'enable_social' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'cherry' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline', 'cherry' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'cherry' ),
					'value'             => '',
					'label'             => esc_html__( 'Content:', 'cherry' ),
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, cherry_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'cherry_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'cherry_register_about_widget' );
function cherry_register_about_widget() {
	register_widget( 'Cherry_About_Widget' );
}
