...
function cherry_utility() {
	return cherry_theme()->get_core()->modules['cherry-utility'];
}
 ...
 * @return void
 */
function cherry_header_class( $classes = array() ) {
	$classes[] = 'site-header';
	$classes[] = get_theme_mod( 'header_layout_type' );
	echo cherry_get_container_classes( $classes, 'header' );
}
 ...
 * @return  void
 */
function cherry_header_container_class( $classes = array() ) {
 ...
 * @return void
 */
function cherry_footer_class( $classes = array() ) {
	$classes[] = 'site-footer';
	$classes[] = get_theme_mod( 'footer_layout_type' );
	echo cherry_get_container_classes( $classes, 'footer' );
}
 ...
 * @return string
 */
function cherry_get_container_classes( $classes, $target = 'content' ) {
	switch ( $target ) {
		case 'header':
 ...
 * @return void
 */
function cherry_primary_content_class( $classes = array() ) {
	echo cherry_get_layout_classes( 'content', $classes );
}
 ...
 * @return string
 */
function cherry_get_layout_classes( $layout = 'content', $classes = array() ) {
	...
	$layout_classes = ! empty( cherry_theme()->layout[ $sidebar_position ][ $sidebar_width ][ $layout ] ) ? cherry_theme()->layout[ $sidebar_position ][ $sidebar_width ][ $layout ] : array();
	...
	$layout_classes = apply_filters( "cherry_{$layout}_classes", $layout_classes );
 ...
 * @return string|void
 */
function cherry_posts_list_class( $classes = array(), $echo = true ) {
	$layout_type      = get_theme_mod( 'blog_layout_type', cherry_theme()->customizer->get_default( 'blog_layout_type' ) );
	$layout_type      = ! is_search() ? $layout_type : 'default';
	$sidebar_position = get_theme_mod( 'sidebar_position', cherry_theme()->customizer->get_default( 'sidebar_position' ) );
	...
	foreach ( $sidebars as $sidebar ) {
		if ( cherry_widget_area()->is_active_sidebar( $sidebar ) ) {
			$has_sidebars = true;
		}
	}
	...
	$classes = apply_filters( 'cherry_posts_list_class', $classes );
