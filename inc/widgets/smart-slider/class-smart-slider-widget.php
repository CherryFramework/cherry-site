...
if ( ! class_exists( 'Cherry_Smart_Slider_Widget' ) ) {

	class Cherry_Smart_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display smart slider on your site.', 'cherry' );
			$this->widget_id          = 'cherry_widget_smart_slider';
			$this->widget_name        = esc_html__( 'Smart Slider', 'cherry' );
			...
			$this->settings           = array(
				...
				'mode' => array(
					...
					'value'   => 'slider',
					'options' => array(
						'slider'  => esc_html__( 'Slider', 'cherry' ),
						'gallery' => esc_html__( 'Gallery', 'cherry' ),
					),
					'label' => esc_html__( 'Mode', 'cherry' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width (less than 100% for `Gallery` mode)', 'cherry' ),
				),
				'height' => array(
					'type'  => 'text',
					'value' => '800',
					'label' => esc_html__( 'Slider height', 'cherry' ),
				),
				'orientation' => array(
					'type'    => 'select',
					'size'    => 1,
					'value'   => 'horizontal',
					'options' => array(
						'horizontal' => esc_html__( 'Horizontal', 'cherry' ),
						'vertical'   => esc_html__( 'Vertical', 'cherry' ),
					),
					'label' => esc_html__( 'Slider orientation', 'cherry' ),
				),
				'slide_distance' => array(
					...
					'max_value' => 100,
					'min_value' => 0,
					'label'     => esc_html__( 'Slide distance(px)', 'cherry' ),
				),
				'slide_duration' => array(
					...
					'min_value'  => 100,
					'step_value' => 100,
					'label'      => esc_html__( 'Slide duration(ms)', 'cherry' ),
				),
				'slide_fade' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use fade effect?  (only for `Slider` mode)', 'cherry' ),
				),
				'navigation' => array(
					'type'   => 'switcher',
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Use navigation?', 'cherry' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'cherry' ),
						'false_toggle' => esc_html__( 'Off', 'cherry' ),
				...
				'fade_navigation' => array(
					...
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Indicates whether the arrows will fade in only on hover', 'cherry' ),
				...
				'pagination' => array(
					...
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use pagination?', 'cherry' ),
				),
				'autoplay' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use autoplay?', 'cherry' ),
				),
				'autoplay_delay' => array(
					...
					'min_value'  => 2000,
					'step_value' => 500,
					'label'      => esc_html__( 'Autoplay delay(ms)', 'cherry' ),
				),
				'fullScreen' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display fullScreen button?', 'cherry' ),
				),
				'shuffle' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Indicates if the slides will be shuffled', 'cherry' ),
				),
				'loop' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use infinite scrolling?', 'cherry' ),
				),
				'thumbnails' => array(
					'type'   => 'switcher',
					'value'  => 'false',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display thumbnails?', 'cherry' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'cherry' ),
						'false_toggle' => esc_html__( 'Off', 'cherry' ),
				...
				'thumbnails_position' => array(
					...
					'options' => array(
						...
						'bottom' => esc_html__( 'Bottom', 'cherry' ),
						'right'  => esc_html__( 'Right', 'cherry' ),
						'left'   => esc_html__( 'Left', 'cherry' ),
					),
					'label'  => esc_html__( 'Sets the position of the thumbnail scroller', 'cherry' ),
				...
				'thumbnails_arrows' => array(
					...
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display thumbnails arrows?', 'cherry' ),
					'master' => 'thumbnails_attr',
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( $posts_query ) {
				...
				$html = '<h4>' . esc_html__( 'Posts for slider not found', 'cherry' ) . '</h4>';
			}
		...
		public function render_slider( $posts_query ) {
			...
			$html = '<div class="cherry-smartslider smart-slider__instance invert" ' . $slider_html_attr . '>';
		...
		public function get_query_slider_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'cherry_smart_slider_default_query_args', array(
				'post_type'           => 'post',
				'orderby'             => 'date',
				'order'               => 'DESC',
				'posts_per_page'      => -1,
		...
		public function get_slider_loop( $posts_query ) {
			 ...
			 * @var  array
			 */
			$more_settings = apply_filters( 'cherry_smart_slider_more_button_settings', array(
				'width'           => '90%',
				'horizontal'      => '5%',
				'vertical'        => '58%',
				'show_transition' => 'left',
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				$html .= '<div class="smart-slider__item sp-slide">';
					...
					$html .= $this->utility->media->get_image( array(
						'size'        => 'cherry-thumb-xl',
						'mobile_size' => 'cherry-thumb-l',
						'class'       => 'sp-image',
	...
	add_action( 'widgets_init', 'cherry_register_smart_slider_widgets' );
	function cherry_register_smart_slider_widgets() {
		register_widget( 'Cherry_Smart_Slider_Widget' );
	}
}
